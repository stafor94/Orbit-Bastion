(function () {
  ...
  const ENEMY_DEFS = {
    ...
    colossus: { name: "브루드 콜로서스", hp: 3750, speed: 19, reward: 50, radius: 28, color: "#ff5e6c", armor: 5, boss: true },
  };
  ...
  const STAGE_RULES = [
    ...
    {
      summary: "갑각형 적과 유충 무리가 동시에 몰려옵니다.",
      rule: "브루트 체력 증가",
      group: { brute: 1.2 },
      hp: { brute: 1.18 },
    },
    ...
    },
    {
      ...
      hp: { swarming: 1.08 },
      extraGroups: [{ wave: 2, type: "swarming", count: 8, gap: 0.18 }],
    },
    {
      summary: "외계 둥지 균열에서 중갑 개체가 계속 증식합니다.",
      rule: "갑각 적 체력 증가",
      group: { brute: 1.45, lurker: 1.15 },
      hp: { brute: 1.28, colossus: 1.2 },
      extraGroups: [{ wave: 3, type: "brute", count: 1, gap: 1.0 }],
    },
    {
      summary: "궤도 잔해 사이로 빠른 생명체와 갑각 무리가 흩어져 침투합니다.",
      rule: "혼합 웨이브 강화",
      group: { skitter: 1.35, brute: 1.25, lurker: 1.2 },
      hp: { skitter: 1.12, brute: 1.18 },
      extraGroups: [{ wave: 4, type: "skitter", count: 5, gap: 0.32 }],
    },
    {
      summary: "하이브 심장부에서 모든 외계 생명체가 강화된 상태로 진격합니다.",
      rule: "최종 총력전",
      group: { swarming: 1.4, skitter: 1.35, brute: 1.5, lurker: 1.25 },
      hp: { lurker: 1.18, skitter: 1.18, brute: 1.35, colossus: 1.35 },
      bossCooldown: 0.72,
      extraGroups: [
        { wave: 3, type: "swarming", count: 10, gap: 0.16 },
  ...
  const DIFFICULTY_DEFS = {
    ...
    normal: { label: "Normal", hp: 1, reward: 1, desc: "적 체력 100%" },
    hard: { label: "Hard", hp: 2, reward: 1, desc: "적 체력 200%" },
    hell: { label: "Hell", hp: 3, reward: 1, desc: "적 체력 300%" },
  };
